...
#As long as we detect one keypoint per side (shoulder, elbow and wrist) we keep the image.
...
#As long as there is visibility of 
def is_good_deadlift_image(keypoints, vis_threshold=0.5):
    # Define required keypoints for visibility check
    lowerbody_keypoints = {
        'hips': [mp_pose.PoseLandmark.LEFT_HIP, mp_pose.PoseLandmark.RIGHT_HIP],
    ...
    upperbody_keypoints = {
        'shoulders': [mp_pose.PoseLandmark.LEFT_SHOULDER, mp_pose.PoseLandmark.RIGHT_SHOULDER],
        'elbows':  [mp_pose.PoseLandmark.LEFT_ELBOW, mp_pose.PoseLandmark.RIGHT_ELBOW],
        'ears': [mp_pose.PoseLandmark.LEFT_EAR, mp_pose.PoseLandmark.RIGHT_EAR]
    }
...
def is_good_squat_image(keypoints, vis_threshold=0.5):
    # Define required keypoints for visibility check
    lowerbody_keypoints = {
        'hips': [mp_pose.PoseLandmark.LEFT_HIP, mp_pose.PoseLandmark.RIGHT_HIP],
        'knees': [mp_pose.PoseLandmark.LEFT_KNEE, mp_pose.PoseLandmark.RIGHT_KNEE],
...
clean_data(r"frames\train\deadlifting", r"cleaned_frames\train\deadlifting", exercise_check_fn=is_good_deadlift_image)
clean_data(r"frames\val\deadlifting", r"cleaned_frames\val\deadlifting", exercise_check_fn=is_good_deadlift_image)

clean_data(r"frames\train\squat", r"cleaned_frames\train\squat", exercise_check_fn=is_good_squat_image)
clean_data(r"frames\val\squat", r"cleaned_frames\val\squat", exercise_check_fn=is_good_squat_image)
